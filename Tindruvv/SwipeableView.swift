...
class SwipeableView: UIView {
    ...
    static var swipePercentageMargin: CGFloat = 0.3
    ...
    private func endedPanAnimation() {
        if let dragDirection = dragDirection, dragDirection == .right || dragDirection == .bottomRight || dragDirection == .topRight, dragPercentage >= SwipeableView.swipePercentageMargin {
        ...
        if let dragDirection = dragDirection, dragDirection == .bottomLeft || dragDirection == .topLeft || dragDirection == .left {
            self.delegate?.didEndSwipe(onView: self, direction: dragDirection)
        }
    }
